...
const strikeDip = function(d){
  ...
  const dip = df(d.dip);
  return <span><span className="strike">{strike}º</span> <span className="dip">{dip}º</span></span>;
};
...
class GroupedAttitudeControl extends React.Component {
  ...
  render() {
    ...
    return <div>
      ...
      <p>
        <Button intent={Intent.DANGER} iconName='ungroup' onClick={this.shouldDestroyGroup}>Ungroup</Button>
      </p>
    </div>;
  }
...
class DataViewer extends React.Component {
  ...
  componentDidMount() {
    if ((this.props.data == null)) {
      return this.setState({content: <p>Hover over data to display fit statistics.</p>});
    } else {
  ...
  render() {
    ...
    return <div>
      <h4>{grouped ? 'Group' : 'Attitude'} {this.props.data.id}</h4>
      ...
      <div className="data-container">
        ...
        <img src={`${window.server_url}/elevation/attitude/${this.props.data.id}/axis-aligned.png`} />
      </div>
    </div>;
  }
}
